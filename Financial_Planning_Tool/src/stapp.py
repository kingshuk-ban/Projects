...
utils.output.print_header("The Tortoise Finance")
...
income = st.sidebar.number_input("Annual Income (Pre-tax)", 10000.0, format="%f")
...
deductions = st.sidebar.number_input("Total pre-tax deductions", 0.0, income, format="%f")
...
st.subheader("Assets")
a = Assets()
stocks = st.sidebar.number_input("Stock Investments", 0.1)
a.add("Stocks", stocks, STOCK_GROWTH_RATE)
cash = st.sidebar.number_input("Cash and Equivalent", 0.1)
a.add("Cash", cash, CASH_GROWTH_RATE)
bonds = st.sidebar.number_input("Bonds and similar", 0.1)
a.add("Bonds", bonds, BOND_GROWTH_RATE)
